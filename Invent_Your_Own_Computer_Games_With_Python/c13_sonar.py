...
def getNewBoard():
    ...
    for x in range(60): # The main list is a list of 60 lists.
        board.append([])
        for y in range(15): # Each list in the main list has 15 single-character string.
...
Enter the coordinates to drop a sonar devise. The ocean map will be marked with
how far away the nearest chest is, or an X if it is beyond the sonar device's
range. For example, the C marks are where chest are. The sonar device shows a
...
while True:
    ...
    while sonarDevices > 0:
        ...
        previousMoves.append([x, y]) # We track all moves so that sonar devices can be updated.
